...
if [ -z "$adbPath" ]; then
	echo "adb not found. You can install it via your package manager."
	exit 1
fi

# Check if a device is connected and authorized
if [ "$($adbPath get-state 2>/dev/null)" != "device" ]; then
	echo "No device connected or authorized. Please connect your device and allow USB-Debugging."
	exit 1
fi

# output all installed packages on the device
echo "Scanning installed apps on the device..."
installed_packages=$($adbPath shell pm list packages | cut -d: -f2)
...
# 4. Uninstall apps, but ONLY if they were found in the live scan
...
do
	...
	if echo "$installed_packages" | grep -q "^$i$"; then
		...
		# Skip uninstalling if the package is not found
		echo "Skipping (not found): $i"
	fi
done
